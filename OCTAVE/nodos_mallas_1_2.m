...
for F = 1:L
  ...
  if O == 1   % Analisis Nodos
    input("NODO TERMINAL POSITIVO? ")
  elseif  O == 2 % Analisis Mallas
    input("MALLA TERMINAL POSITIVO? ")
  endif
  P = ans;
  
  if O == 1   % Analisis Nodos
    input("NODO TERMINAL NEGATIVO? ")
  elseif  O == 2 % Analisis Mallas
    input("MALLA TERMINAL NEGATIVO? ")
  endif
  N = ans;
  
  if P>Q || N>Q % Uno de los dos Nodos excede el numero de Nodos 
    disp("ERROR NUMERO EXCEDE NUMERO DE NODOS/MALLAS")
    return
  endif
